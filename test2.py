...
# 已知人脸数据库路径
known_faces_db_path = "facedata"
confidence_threshold = 0.4  # 设定置信度阈值
...
def detect_and_draw_faces(frame, backend='dlib'):
    try:
        ...
        for face in faces:
            ...
            result = DeepFace.find(img_path=face_img, db_path=known_faces_db_path, enforce_detection=False,
                                   model_name="Dlib", distance_metric='cosine')
            if len(result) == 0 :
                # 未知人脸
                face_info.append((x, y, w, h, 'unknown'))
            else:
                # 已知人脸
                face_info.append((x, y, w, h, 'known'))
        return face_info
...
def process_frame_queue(frame_queue, result, lock, stop_event):
    while not stop_event.is_set():
        try:
            ...
            face_info = detect_and_draw_faces(frame)

            # 使用锁更新结果
            with lock:
                result['value'] = face_info
...
cap = cv2.VideoCapture(get_DroidCam_url('192.168.10.114', 4747, '720p'))
...
while True:
    ...
    if ret:
        ...
        # 读取处理结果并画框
        with lock:
            face_info = result['value']
        for (x, y, w, h, face_type) in face_info:
            color = (0, 255, 0) if face_type == 'known' else (0, 0, 255)
