...
def save_stranger_data(stranger_queue, db_lock, stop_event):
    last_save_time = 0  # 上次保存时间
    while not stop_event.is_set():
        try:
            stranger_date, stranger_frame = stranger_queue.get(timeout=1)

            current_time = time.time()
            if current_time - last_save_time >= 30:
                # 保存摔倒信息到数据库并获取插入后的fid
                with db_lock:
                    cursor.execute("INSERT INTO stranger(detect_date) VALUES (%s)", (stranger_date,))
                    db.commit()
                    sid = cursor.lastrowid

                # 使用fid作为图像名称
                image_name = f'{sid}.jpg'
                image_path = f'stranger/{image_name}'

                # 保存图像到本地
                cv2.imwrite(image_path, stranger_frame)

                # 上传图像到MinIO
                minio_client.fput_object(bucket_name, f'stranger/{image_name}', image_path, content_type='image/')

                last_save_time = current_time  # 更新上次保存时间

                print('Saved stranger data')
            stranger_queue.task_done()
        except queue.Empty:
            continue


stranger_queue = queue.Queue()
db_lock = threading.Lock()
# 用于停止线程的事件
stop_event = threading.Event()

# 启动保存数据线程
saving_thread = threading.Thread(target=save_stranger_data, args=(stranger_queue, db_lock, stop_event))
...
class Face_Recognizer:
    ...
    async def process(self, websocket):
        ...
        if self.get_face_database():
            while stream.isOpened():
                    ...
                    # 6.2.2 人脸数增加 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                #sql
                                face_img = img_rd[faces[k].top():faces[k].bottom(), faces[k].left():faces[k].right()]
                                stranger_date = datetime.now()
                                stranger_queue.put((stranger_date, face_img))
                                unknown_face_filename = f"unknown_face.jpg"
                                logging.info(f"Unknown face saved as {unknown_face_filename}")
                ...
                buffer = cv2.imencode('.jpg', img_rd)[1]
    ...
    def run(self):
        ...
        start_server = websockets.serve(self.process, "0.0.0.0", 8082)
        # cap.release()
